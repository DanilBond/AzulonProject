...
namespace Azulon.Tests.EditMode.Presentation.Gameplay
{
    public sealed class GameSessionPresenterTests
    {
        ...
        {
            var offer = _presenter.CreateViewData().Offers[0];
        ...
        {
            var staleOfferId = _presenter.CreateViewData().Offers[0].OfferId;
        ...
        {
            var firstOffer = _presenter.CreateViewData().Offers[0];
            _presenter.PurchaseOffer(firstOffer.OfferId);
            _presenter.ClaimQuest(_context.EmberCommission.Id);
            _presenter.AdvanceToNextVisitor();

            var flameOrbOffer = _presenter.CreateViewData().Offers.Single(
                offer => offer.Item.Id == _context.FlameOrb.Id);
            ...
            Assert.That(completed.Offers.All(offer => !offer.CanPurchase), Is.True);
            Assert.That(
